...
def get_local_response(user_message):
    ...
    if '内存边界值' in message:
        return '内存调优的边界值指的是一系列关键阈值和限制。当内存使用量接近或超过这些值时，系统的行为会发生质的变化，通常表现为性能急剧下降或直接崩溃。对应用程序而言：可能是堆内存大小、垃圾回收频率的阈值。对操作系统而言：可能是可用物理内存、Swap使用率的阈值。对容器而言：可能是内存限制的阈值。处理这些边界值问题的核心思想是：在问题发生之前，通过监控和设定缓冲区间，提前预警或处理，避免系统直接撞上“墙”。'
